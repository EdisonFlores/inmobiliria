...
const images = { 'inmobiliaria-hero.png': `${BASE_URL}inmobiliaria-hero.png` };
...
function Brand({ settings }) {
  const name = settings.business_name || 'Inmobiliaria';
  return <a className={`brand ${settings.logo_url ? 'brand-with-logo' : ''}`} href={pageUrl('/')} aria-label={name}>{settings.logo_url ? <img className="brand-logo" src={cloudinaryVersion(settings.logo_url, 'c_limit,w_440,q_auto:eco,f_auto')} alt={name} decoding="async"/> : <><span className="brand-mark"><Building2 size={22}/></span><span className="brand-wordmark"><strong>Inmobiliaria</strong><small>Bienes <em>raíces</em></small></span></>}</a>;
}
...
function HomePage({ t, settings, lang }) {
  ...
  const heroImage = cloudinaryVersion(settings.hero_desktop_url, 'c_limit,w_1800,q_auto:good,f_auto') || asset('inmobiliaria-hero.png');
  const heroMobileImage = cloudinaryVersion(settings.hero_mobile_url, 'c_limit,w_900,q_auto:good,f_auto');
  const aboutImage = cloudinaryVersion(settings.about_image_url, 'c_limit,w_1400,q_auto:good,f_auto') || asset('inmobiliaria-hero.png');
  return <>
...
function PropertyDetailPage({ t, lang, identifier, settings }) {
  ...
  return <>
    ...
    <section className="detail-content"><div className="container">
      <MediaCarousel t={t} media={imageMedia} fallbackImage={property.image || asset('inmobiliaria-hero.png')}/>
...
function SoldCaseCard({ item, t, lang }) {
  ...
  const photos = item.media?.length
    ...
    : [{ src: item.image || asset('inmobiliaria-hero.png'), label: t.salesProcess }];
...
function PrivacyPolicyPage({ lang, settings }) {
  const es = lang === 'es';
  const business = settings.business_name || 'Inmobiliaria';
